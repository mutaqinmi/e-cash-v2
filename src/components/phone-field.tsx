...
export default function PhoneField(props: {label: string; defaultValue?: string | null}){
    const [value, setValue] = useState<string>(props.defaultValue ?? "");

    return (
        <div className="my-4 flex flex-col relative">
            <input required type="text" name={props.label.toLowerCase()} id={props.label.toLowerCase()} className="outline-none border border-gray-400 py-3 px-3 rounded-md peer" value={value} onChange={(e) => setValue(formatPhoneNumber(e.currentTarget.value))}/>
