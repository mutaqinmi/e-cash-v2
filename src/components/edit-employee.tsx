...
export default function EditEmployee(props: {popupController?: (show: boolean | null) => void; successSnackbarController?: (message: string | null) => void; errorSnackBarMessage?: (message: string | null) => void; onSuccess?: () => void; data: table.employeeType}){
    ...
    return <div className="w-screen h-screen fixed top-0 left-0 z-50 bg-black bg-opacity-25 flex justify-center items-center">
        <div className="w-[25rem] p-4 bg-white rounded-md relative">
            ...
            <div>
                ...
                <span className="text-sm">Edit data pegawai.</span>
            </div>
            <div className="mt-8">
                <Form action={updateEmployeeHandler} formMethod="POST">
                    <InputField label="Nama Pegawai" defaultValue={props.data.full_name}/>
                    <InputField label="Username" defaultValue={props.data.user_name}/>
                    <Dropdown label="Hak Akses" defaultValue={props.data.role}>
                        <option value="Administrator">Administrator</option>
                        <option value="Kasir">Kasir</option>
                    </Dropdown>
                    <Button className="w-full mt-8" label="Edit Pegawai" formButton></Button>
                </Form>
            </div>
        </div>
    </div>
